...
function AssociateDashboard({ user, activeTab }) {
  ...
  const acceptJob = async (issueId) => {
    ...
    if (!cost || cost <= 0) {
      setMsg('Please enter a cost estimate');
      setMsgType('error');
      return;
    }
    try {
      const res = await fetch(`http://localhost:5000/api/associate/issues/${issueId}/accept`, {
        method: 'POST',
      ...
      if (!res.ok) {
        setMsg(data.message || 'Failed to accept job');
        setMsgType('error');
        return;
      }
      setMsg('Job accepted! You can now proceed with the work.');
      setMsgType('success');
      setCostMap(prev => ({ ...prev, [issueId]: '' }));
      loadIssues();
    } catch (err) {
      setMsg('Failed to accept job');
      setMsgType('error');
    }
  };
  ...
  const rejectJob = async (issueId) => {
    ...
    try {
      const res = await fetch(`http://localhost:5000/api/associate/issues/${issueId}/reject`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
      ...
      const data = await res.json().catch(()=>({}));
      if (!res.ok) {
        setMsg(data.message || 'Failed to reject job');
        setMsgType('error');
        return;
      }
      setMsg('Job rejected');
  ...
  const completeJob = async (issueId) => {
    ...
    if (!notes || !notes.trim()) {
      setMsg('Please provide completion notes');
      setMsgType('error');
      return;
    }
    try {
      const res = await fetch(`http://localhost:5000/api/associate/issues/${issueId}/complete`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + token },
      ...
      const data = await res.json().catch(()=>({}));
      if (!res.ok) {
        setMsg(data.message || 'Failed to complete job');
        setMsgType('error');
        return;
      }
      setMsg('Job completed and reported to director!');
      setMsgType('success');
      setNotesMap(prev => ({ ...prev, [issueId]: '' }));
      loadIssues();
    } catch (err) {
      setMsg('Failed to complete job');
      setMsgType('error');
    }
  };
  ...
  return (
    <div className="card">
      <div className="card-header">
        <h2>Assigned Jobs</h2>
        <div className="muted">Manage your repair assignments</div>
      </div>
      ...
      <div>
        {loading ? <div className="muted">Loading…</div> : (
          <ul style={{padding:0,margin:0}}>
            {issues.length === 0 && <li className="muted" style={{listStyle:'none'}}>No assigned issues.</li>}
            {issues.map(iss => {
                ...
                <li key={iss._id} style={{ listStyle:'none', margin:0, padding:0 }}>
                  <div style={{
                    ...
                    <div style={{display:'flex',justifyContent:'space-between',alignItems:'flex-start',gap:12,flexWrap:'wrap'}}>
                      <div style={{flex:1, minWidth:200}}>
                        <div style={{display:'flex',alignItems:'center',gap:8,flexWrap:'wrap',marginBottom:6}}>
                          <div style={{fontSize:16,fontWeight:700,lineHeight:1.2}}>{iss.title || 'Issue'}</div>
                          <span style={{background:'#f1f5f9',color:'#334155',fontSize:11,padding:'4px 8px',borderRadius:14}}>{iss.urgency === 'urgent' ? '🔴 Urgent' : 'Not urgent'}</span>
                        ...
                        <div style={{fontSize:12,color:'#475569',marginTop:8}}>
                          <div>👤 <strong>{tenantName}</strong> • Unit {unitNumber}</div>
                          <div>📍 {address}</div>
                      ...
                      {iss.cost && (
                        <div style={{padding:10, background:'#f0f9ff', borderRadius:8, textAlign:'center', minWidth:100}}>
                          <div style={{fontSize:11, color:'#0369a1', textTransform:'uppercase'}}>Cost Estimate</div>
                    ...
                    <div style={{background:'#f8fafc', borderRadius:8, padding:12}}>
                      <div style={{fontSize:13, fontWeight:600, marginBottom:6, color:'#0f172a'}}>Issue Details</div>
                      ...
                      {isAssigned && (
                        <>
                          ...
                          <div style={{display:'flex', gap:8}}>
                            <button className="btn" disabled={!costMap[iss._id]} onClick={()=>acceptJob(iss._id)} style={{flex:1}}>✅ Accept Job</button>
                            <button className="btn ghost" onClick={()=>rejectJob(iss._id)} style={{flex:1}}>❌ Reject Job</button>
                          </div>
                        </>
                      )}
                      {isInProgress && (
                        <>
                          ...
                          />
                          <button className="btn" disabled={!notesMap[iss._id]} onClick={()=>completeJob(iss._id)}>✔️ Report Work Completed</button>
                        </>
                      )}
                      {isResolved && (
                        <div style={{background:'#dcfce7', padding:10, borderRadius:8, color:'#166534', fontSize:13, fontWeight:600}}>
                          ✅ Work completed on {iss.completionDate ? new Date(iss.completionDate).toLocaleDateString() : 'date'}
                        </div>
                      )}
                    </div>
                  </div>
